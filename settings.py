...
if WAIT_FOR_CAPTURE:
    try:
        WAIT_FOR_CAPTURE_TIMEOUT = int(os.getenv("WAIT_FOR_CAPTURE_TIMEOUT", "300"))
    except ValueError:
        print(
...
if (
    ...
    and os.getenv("CLASSES_OF_INTEREST_PATH")
    and os.getenv("CONFIDENCE_THRESHOLD")
):
    CLASSES_PATH = os.getenv("CLASSES_PATH")
    CLASSES_OF_INTEREST_PATH = os.getenv("CLASSES_OF_INTEREST_PATH")
    CONFIDENCE_THRESHOLD = float(os.getenv("CONFIDENCE_THRESHOLD"))
else:
    print("CLASSES_PATH, CLASSES_OF_INTEREST_PATH and/or CONFIDENCE_THRESHOLD not set or invalid.")
    ENVS_READY = False

# Configs for YOLO (You Only Look Once) detector
if DETECTOR == "yolo":
    if os.getenv("YOLO_WEIGHTS_PATH") and os.getenv("YOLO_CONFIG_PATH"):
        YOLO_WEIGHTS_PATH = os.getenv("YOLO_WEIGHTS_PATH")
        YOLO_CONFIG_PATH = os.getenv("YOLO_CONFIG_PATH")
    else:
        print("YOLO_WEIGHTS_PATH and/or YOLO_CONFIG_PATH not set or invalid.")
        ENVS_READY = False

# Configs for YOLO (You Only Look Once) detector
if DETECTOR == "yolov8":
    if os.getenv("YOLOV8_MODEL_PATH"):
        YOLOV8_MODEL_PATH = os.getenv("YOLOV8_MODEL_PATH")
    else:
        print("YOLOV8_MODEL_PATH not set or invalid.")
        ENVS_READY = False
...
try:
    ...
    print(
        "Invalid value for ENABLE_CONSOLE_LOGGER and/or "
...
VIDEO_WRITING_DIRECTORY = os.getenv("VIDEO_WRITING_DIRECTORY", "")
...
try:
    ...
    print(
        "Invalid value for DEBUG_WINDOW_SIZE. It should be a 2-tuple: (width, height)."
    )
    ENVS_READY = False
...
if not ENVS_READY:
    raise Exception(
        "One or more environment variables are either invalid or not set. "
